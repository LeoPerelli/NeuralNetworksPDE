# -*- coding: utf-8 -*-
"""
Created on Sat Apr 10 09:33:36 2021
...
# f = 4sin(x)sin(y) + 2cos(x)sen(y) + sin(x)cos(y)
# mu = 3
# b = [2; 1]
# sig = -2
...
forcing = lambda x: 4 * np.sin(x[:,0]) * np.sin(x[:,1]) + 2 * np.cos(x[:,0]) * np.sin(x[:,1]) + np.sin(x[:,0]) * np.cos(x[:,1]) 


#numerical options
num_PDE = 500      #points of numerical sol
num_BC = 20        #points of numerical BC
num_fit = 50       #points of fit (the training data)
...
x_PDE = tf.random.uniform(shape = [num_PDE, 2], minval = [0, 0], maxval = [domain_W1, domain_W2], dtype=data_type)

#grid of the fitting sol (training data)
x_fit = tf.random.uniform(shape = [num_fit, 2], minval = [0, 0], maxval = [domain_W1, domain_W2], dtype=data_type)

#grid of the test sol
x_test = tf.random.uniform(shape = [num_test, 2], minval = [0, 0], maxval = [domain_W1, domain_W2], dtype=data_type)

#Boundary grid
#random points in layer ([0,0],[0,2pi])  (have two coords)
x_BC_x0 = tf.random.uniform(shape = [num_BC,2], minval = [0, 0], maxval = [0, domain_W2], dtype=data_type)
#random points in layer ([2pi,0],[2pi,2pi])  (have two coords)
x_BC_x1 = tf.random.uniform(shape = [num_BC,2], minval = [domain_W1, 0], maxval = [domain_W1, domain_W2], dtype=data_type)
#random points in layer ([0,0],[2pi,0])  (have two coords)
x_BC_y0 = tf.random.uniform(shape = [num_BC,2], minval = [0, 0], maxval = [domain_W1, 0], dtype=data_type)
#random points in layer ([0,2pi],[2pi,2pi])  (have two coords)
x_BC_y1 = tf.random.uniform(shape = [num_BC,2], minval = [0, domain_W2], maxval = [domain_W1, domain_W2], dtype=data_type)
...
losses = [
          ns.LossMeanSquares('fit', lambda: tf.squeeze(model(x_fit)) - u_fit, normalization = u_norm, weight=0.1),   #fit loss, MSE(output of NN - exact sol)
