...
ns.minimize(pb, 'scipy', 'BFGS',                                       num_epochs = 9000, options={'gtol': 1e-100})
...
if(save_model):
    ...
    minmax_bounds['min_train'] = float(min_train)
    minmax_bounds['max_train'] = float(max_train)
    
    with open('current_minmax.json', 'w') as fp:
        json.dump(minmax_bounds, fp)
    
    ns.utils.export_ANN(model, 'current_model')
